...
class HINGCN_Dense(nn.Module):
    def __init__(self,
        ...
        self.mp_agg = mpaggr_class(out_dim,n_head=self.n_mp+int(self.bias), dropout=self.dropout,batchnorm=self.batchnorm,)
    ...
    def forward(self, ids, train=True):
        ...
        for mp in range(self.n_mp):
            ...
            for i in range(self.depth):
                ...
                all_feats = F.dropout(all_feats, self.dropout, training=self.training)
